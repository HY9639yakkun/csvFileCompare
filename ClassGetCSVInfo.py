            ...
            for filename in list_filenames:
                ...
                try:
                    ...
                    if ext == '.csv':
                        ...
                        self.file_error_tuplelist.append((target_file_PATH1,'csvファイルではありません'))
    ...
    def input_csvfiles_client(self):
        ...
        if flag_run:
            ...
            for filename in list_filenames:
                ...
                if ext == '.csv':
                    ...
                    try:
                        ...
                        print(traceback.format_exc())
                        self.file_error_tuplelist.append((self.dir_path_pri + '/' + filename,'存在しません'))
                        continue
                    ...
                    try:
                        ...
                        print(e, 'error occurred')
                        print(traceback.format_exc())
                        self.file_error_tuplelist.append((target_file_PATH2,'情報が取得できません'))
                        
                    #CSVファイルを閉じる
                    csv_file.close()
                else:
                    print('{}はcsvファイルではありません。'.format(filename))
                    self.file_error_tuplelist.append((target_file_PATH2,'csvファイルではありません'))
            ...
            for error_obj in self.info_pri_obj_dict.values():
                self.file_error_tuplelist.append((self.dir_path_client + '/' + error_obj.filename,'存在しません'))
