...
class StructInfoForCSVCmp:
    ...
    #クラス変数
    #チェックIDに対応する関数を辞書に格納
    ID2FunctionDir={'sample':input_info_list_sample,\
    ...
    def input_info_list_pri(self,csvdata_1row):
        ...
        time_start_str=csvdata_1row[2]#出社時刻
        time_end_str=csvdata_1row[3]#勤務終了時間
        self.info_tuplelist_pri.append((date_str,status_str,time_start_str,time_end_str))#タプル追加
    ...
    def cmp_info_list(self):
        ...
        比較を行う
        '''
        for info_tuple_pri, info_tuple_client1 in zip(self.info_tuplelist_pri, self.info_tuplelist_client):
            #1行ごとの比較結果を格納
