...
def writeResultToHtml(outputFile,info_obj,date_num_all):
    ...
    indresult2HtmlIdTuple  = ('ind_result_NG','ind_result_OK','ind_result_Nothing')#個別結果用Html ID
    dispMessTuple  = ('NG','OK','-','PASS')#表示するメッセージ
    result2HtmlIdTuple  = ('result_NG','result_OK','result_NOTHING','result_PASS')#総合結果用Html ID
    ...
    for date_num in range(1,date_num_all+1):
        ...
        if date_num <= info_num_all:
            ...
            outputFile.write('<td class="{}"><p>{}</p></td>\n'.\
            format(indresult2HtmlIdTuple[result_ind],dispMessTuple[result_ind]))
        else:
            #結果が日数に対して不足
            outputFile.write('<td class="{}"><p>{}</p></td>\n'.\
            format(indresult2HtmlIdTuple [2],dispMessTuple[2]))
    #個人ごとの総合結果書き込み
    ...
    outputFile.write('<td class="{}"><p>{}</p></td>\n'.\
    format(result2HtmlIdTuple[cmp_result_num_for_report],dispMessTuple[cmp_result_num_for_report]))
    outputFile.write('</tr>\n')
...
def run_csv_cmp():
    ...
    if flag_run:
        ...
        try:
            ...
            outputFile.write('<p><font>比較パターン : {}</font></p>\n'.format(get_csv_obj.client_type))
            outputFile.write('<p><font>PATH1 : {}</font></p>\n'.format(get_csv_obj.dir_path_pri))
            outputFile.write('<p><font>PATH2 : {}</font></p>\n'.format(get_csv_obj.dir_path_client))
            outputFile.write('<br>\n')
            #######################################################
            ...
            for date_str, ele in zip(c1.itermonthdates(thisYear, thisMomth),c1.itermonthdays2(thisYear, thisMomth)):
                if not(ele[0] == 0):
                    date_num_all=date_num_all+1
                    youbiStr=dayNum2YoubiTuple [ele[1]]#曜日
                    outputFile.write('<td class="date"><p>{}({})</p></td>\n'.format(date_str, youbiStr))
            ...
            for info_obj in get_csv_obj.info_obj_list:
                #結果の書き込み##################################################
                cmp_result_num_for_report,ngFileName=writeResultToHtml(outputFile,info_obj,date_num_all)
                if all_path_flag:
            ...
            if (ngFileLen == 0):
                ...
                outputFile.write('<h3>比較結果NGファイルリスト({}/{})</h3>\n'.format(ngFileLen,info_obj_len))
                outputFile.write('<ul>\n')
            ...
            if not(file_error_len == 0):
                outputFile.write('<h3>比較チェック不可ファイルリスト({})</h3>\n'.format(file_error_len))
